...
void print_binary_file(csr_graph to_print, string base) {
  ...
  if (!node_array_file.is_open()) assert(0);
  
  if (to_print.edges > MAX_EDGES) {
    cout << "Writing node byte length of:    " << (to_print.nodes + 1) * sizeof(unsigned long) << endl;
    node_array_file.write((char *)to_print.node_array, (to_print.nodes + 1) * sizeof(unsigned long));
  } else {
    cout << "Writing node byte length of:    " << (to_print.nodes + 1) * sizeof(unsigned int) << endl;
    unsigned int* tmp_node_array = (unsigned int*) malloc(sizeof(unsigned int) * (to_print.nodes + 1));
    for (unsigned int n = 0; n < to_print.nodes + 1; n++) {
      tmp_node_array[n] = (unsigned int) to_print.node_array[n];
    }
    node_array_file.write((char *)tmp_node_array, (to_print.nodes + 1) * sizeof(unsigned int));
  }
  node_array_file.close();

  ofstream edge_array_file;
  edge_array_file.open(base + "edge_array.bin", ios::out | ios::binary);
  if (!edge_array_file.is_open()) assert(0);

  if (to_print.edges > MAX_EDGES) {
    cout << "Writing edge byte length of:    " << (to_print.edges) * sizeof(unsigned long) << endl;
    edge_array_file.write((char*)to_print.edge_array, to_print.edges * sizeof(unsigned long));
  } else {
    cout << "Writing edge byte length of:    " << (to_print.edges) * sizeof(unsigned int) << endl;
    unsigned int* tmp_edge_array = (unsigned int*) malloc(sizeof(unsigned int) * (to_print.edges));
    for (unsigned int e = 0; e < to_print.edges; e++) {
      tmp_edge_array[e] = (unsigned int) to_print.edge_array[e];
    }
    edge_array_file.write((char*)tmp_edge_array, (to_print.edges) * sizeof(unsigned int));
  }
  edge_array_file.close();

  ofstream edge_values_file;
  edge_values_file.open(base + "edge_values.bin", ios::out | ios::binary);
  if (!edge_values_file.is_open()) assert(0);
  ...
  auto end = std::chrono::system_clock::now();
  chrono::duration<double> elapsed_seconds = end-start;
  cout << "Writing graph elapsed time: " << elapsed_seconds.count() << "s\n\n";
}
...
csr_graph parse_bin_files(string base) {
  ...
  cout << "Verifying graph conversion...\n\tNodes: " << nodes << "\n\tEdges: " << edges << "\n";
  ...
  if (ret.edges > MAX_EDGES) {
    cout << "Reading node byte length of:    " << (ret.nodes + 1) * sizeof(unsigned long) << endl;
    node_array_file.read((char *)ret.node_array, (ret.nodes + 1) * sizeof(unsigned long));
  } else {
    cout << "Reading node byte length of:    " << (ret.nodes + 1) * sizeof(unsigned int) << endl;
  ...
  if (ret.edges > MAX_EDGES) {
    cout << "Reading edge byte length of:    " << (ret.edges) * sizeof(unsigned long) << endl;
    edge_array_file.read((char*)ret.edge_array, (ret.edges) * sizeof(unsigned long));
  } else {
    cout << "Reading edge byte length of:    " << (ret.edges) * sizeof(unsigned int) << endl;
  ...
  edge_values_file.read((char*)ret.edge_values, ret.edges * sizeof(int));
  edge_values_file.close();        
